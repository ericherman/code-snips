...
The challenge involved needing to use a custom allocator. This
...
public:
	...
	const char *host;
	 ...
	 Host_user(const char *id, const char *host) {
		...
		this->host=
		    tracking_strdup(memory_key_b, host ? host : "localhost");
	};
	~Host_user() {
...
typedef Tracking_allocator<pair<const string, Host_user_ptr_list>>
		Name_hu_pair_allocator;
typedef scoped_allocator_adaptor<Name_hu_pair_allocator,
		Host_user_ptr_allocator> Name_to_users_allocator;
typedef unordered_map<string, Host_user_ptr_list, hash<string>,
		equal_to<string>, Name_to_users_allocator> Name_to_users_map;
...
void load_all_users()
{
	...
	all_users[0]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("alice",
							       "10.0.0.1");
	all_users[1]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("alice",
							       "10.1.2.3");
	all_users[2]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("bob",
							       "10.0.0.2");
	all_users[3]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("chris",
							       "10.0.1.1");
	all_users[4]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("dirk",
							       "10.0.1.2");
	all_users[5]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("eve", nullptr);
	all_users[6]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("eve",
							       "10.0.0.2");
	all_users[7]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("eve",
							       "10.0.1.2");
	...
	all_users[10]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("eve",
							       "10.8.0.2");
	all_users[11]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("frances",
							       "10.0.1.2");
	all_users[12]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("glen",
							       "10.8.0.7");
	all_users[13]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("harley",
							       "10.8.0.7");
	all_users[14]=
	    new(tracking_malloc(memory_key_b, size)) Host_user("ishta",
							       "10.8.0.7");
	all_users[15]= NULL;
...
int print_name_to_users(FILE *stream)
{
	...
	int bytes_written= 0;
	for (auto it= (*name_to_users).begin(); it != (*name_to_users).end();
	     ++it) {
		string name= it->first;
		auto list= it->second;
		rv= fprintf(stream, "%s => {\n", name.c_str());
		if (rv < 0)
			goto err_print_name_to_users;
		bytes_written += rv;
